...
const drawStair = (state: State, name: string) => {
  ...
  const {facing: facingConfig, half: halfConfig}: Record<string, string> = paramSplit
    .slice(0, paramSplit.length - 1)
    .split(',')
    .reduce((acc, cur) => ({[cur.split('=')[0]]: cur.split('=')[1], ...acc}), {})

  const topStair = new THREE.Mesh(
    new THREE.BoxBufferGeometry(stairConfig.l, stairConfig.w, stairConfig.h),
    state.cubeMaterials['oak_planks'],
  )
  ...
  const bottomStair = new THREE.Mesh(
    new THREE.BoxBufferGeometry(stairConfig.l, stairConfig.w, stairConfig.h * 2),
    state.cubeMaterials['oak_planks'],
  )
  ...
  const stairMask = new THREE.Mesh(
    new THREE.BoxBufferGeometry(stairConfig.l, stairConfig.w * 2, stairConfig.h * 2),
    maskMaterial,
  )
