...
const flipx = (state: State) => {
  state.objects
    .map(getParent)
    .forEach((obj) => (isntPlane(obj) ? (obj.position.x = invert(obj.position.x)) : null))
}
const flipy = (state: State) => {
  state.objects
    .map(getParent)
    .forEach((obj) => (isntPlane(obj) ? (obj.position.y = invert(obj.position.y)) : null))
}
const flipz = (state: State) => {
  state.objects
    .map(getParent)
    .forEach((obj) => (isntPlane(obj) ? (obj.position.z = invert(obj.position.z)) : null))
}
